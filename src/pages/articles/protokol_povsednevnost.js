...
const url = "https://teatralium.com/articles/protokol_povsednevnost/";
const title =
...
export const DokCover = ({ sizes, showDate = true, ...props }) => (
  <TopCover color="white" sizes={sizes} bordered borderColor="white" {...props}>
    <Section margin="0" align={props.align || "center"}>
      <H1Shadow color="black" size="3.2rem">
      ...
      {showDate && <StyledDate>
        11 июля 2018, текст: <b>Настя Николаева</b>
      </StyledDate>}
    </Section>
  </TopCover>
);
  ...
  render() {
    ...
    return (
      <div>
        ...
        <ArticleHelmet
          url={url}
          image={data.protokol1.sizes.src}
          description="Даже если вы «вообще не театральный человек»."
          title={`${title} | Театралий`}
        />
        <DokCover />
        <Section>
          <P>
            Спустя три месяца после отчетного показа кружка «Протокол. Повседневность» погода наконец позволила надеть пиджак, в котором я выходила на сцену. В кармане оказалось кое-что подороже забытого с прошлого сезона полтинника — шпаргалка финального спектакля с порядком наших выходов и некоторыми моими репликами. Так распорядилась судьба, что текст о том кружке я написала только сейчас, отчего он получился ностальгическим, под стать настроению прощания с сезоном, которое приходит к нам перед летним затишьем. Текст о том, почему{" "}
            <a href="http://meyerhold.ru/protokol-my/">
              кружок «Протокол»
           </a> — это любовь и как Центру Мейерхольда удалось собрать вокруг себя комьюнити москвичей и гостей столицы и организовать их в театр горожан.
          </P>
        </Section>
        <ImgG maxWidth="100%" sizes={data.protokol1.sizes} />
        <Section withHomepageLink>
          ...
          <Video url="//coub.com/embed/1astik?muted=false&autostart=false&originalSize=false&startWithHD=false" />
          <p>
          ...
          <Video id="CZqAOuUrFXM" desc="«Философия повседневности». Авторская песня" />
          ...
          <a
            href="https://www.facebook.com/wwweidenbaum/posts/10216839316601682"
            target="_blank">
            <Img
              src="https://media.giphy.com/media/fMA99Gn2q3mukhSdxQ/giphy.gif"
              maxWidth="400px"
            />
          </a>

          <p>
            Чаще всего люди узнают о кружках на сайте ЦИМа либо от знакомых, которые уже через это прошли, и ведь сарафанный эффект подтверждает, что это важно. За три сезона в кружках поучаствовало уже серьезное количество человек, зафиксировавших себя на каком-то конкретном этапе жизни в конкретном месте. Их опыт — своеобразный источник современных народных обычаев, традиций и повадок, то есть городской культуры для театра. А для них самих эта возможность прочувствовать свою седалищную кость в компании приятных людей оказывается важным моментом самоориентации, идентификации, ощущения собственной причастности к чему бы то ни было. Все это очень ответственно. Новый «Протокол», <a href="https://tickets.meyerhold.ru/tickets.php?event=4833">отчетный показ которого закроет фестиваль перформанса</a> и сезон заодно, посвящен будущему. Возможно, там тоже прозвучат фразы типа «Я хочу уснуть и никогда не проснуться» (от повседневных трудностей бытия сложно отделаться), но, вообще-то, он будет про то, чего мы не знаем, но что пытаемся конструировать и о чем мечтаем. ЦИМ с помощью драматургов, хореографов, режиссеров и художников, которые ведут здесь кружки, дал своим зрителям инструменты и сцену для высказывания на большинство тем, существующих в мире. Эти рассказы уже можно объединять в{" "}<a href="https://www.facebook.com/attn/videos/1714737678561684/ ">проекты типа таких</a>, потенциал у театра горожан огромен, а люди все еще довольно отзывчивы и готовы участвовать во всяком.
          </p>

          <Video id="WJP1qrF3fsU" desc="Отчетный показ кружка «Протокол. Повседневность»" />
          ...
          <Video url="//coub.com/embed/1asv8y?muted=false&autostart=false&originalSize=false&startWithHD=false" />
...
export const pageQuery = graphql`
  query ProtokolImageQuery {
          protokol1: imageSharp(id: {regex: "/protokol/protokol1.jpg/" }) {
          sizes(maxWidth: 1400) {
          ...GatsbyImageSharpSizes
        }
        }
    protokol2: imageSharp(id: {regex: "/protokol/protokol2.png/" }) {
          sizes(maxWidth: 700) {
          ...GatsbyImageSharpSizes
        }
        }
    protokol3: imageSharp(id: {regex: "/protokol/protokol3.png/" }) {
          sizes(maxWidth: 700) {
          ...GatsbyImageSharpSizes
        }
        }
